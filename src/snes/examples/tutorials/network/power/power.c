...
int main(double *retarray, char *bus_data_str, int bus_len, char *gen_data_str, int gen_len, char *branch_data_str, int branch_len, int argc, char ** argv)
{
  PetscErrorCode   ierr;
  ...
  //double           *retarray;
  int              mpi_flag;
  int              rand_int_log_stage;
  char             log_stage_stringname[100];
  ...
  if(!mpi_flag){
    ...
    printf("INITIALIZING MPI USING PETSCINITIALIZE!!!!!!!!!!!!!!!!!");
  }
  //ierr = PetscInitialize(&argc,&argv,"poweroptions",help);CHKERRQ(ierr);
  ierr = MPI_Comm_rank(PETSC_COMM_WORLD,&rank);CHKERRQ(ierr);
  {
    ...
    rand_int_log_stage = rand()%100000;
    sprintf(log_stage_stringname, "Read Data %d", rand_int_log_stage);
    //ierr = PetscLogStageRegister("Read Data",&stage1);CHKERRQ(ierr);
    ierr = PetscLogStageRegister(log_stage_stringname,&stage1);CHKERRQ(ierr);
    ...
    if (!crank) {
      ...
      ierr = PFReadMatPowerData(pfdata, pfdata_file, bus_data_str, bus_len, gen_data_str, gen_len, branch_data_str, branch_len);CHKERRQ(ierr);
    ...
    sprintf(log_stage_stringname, "Create network %d", rand_int_log_stage);
    //ierr = PetscLogStageRegister("Create network",&stage2);CHKERRQ(ierr);
    ierr = PetscLogStageRegister(log_stage_stringname,&stage2);CHKERRQ(ierr);
    ...
    PetscInt nloc;
    PetscScalar *array;
    ierr = VecGetSize(X,&nloc);CHKERRQ(ierr);
    ierr = VecGetArray(X, &array);CHKERRQ(ierr);

    //retarray = (double *)malloc(nloc*sizeof(double));
    memcpy(retarray, array, nloc*sizeof(double));
