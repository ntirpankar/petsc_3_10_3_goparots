...
#if 1
PetscErrorCode PFReadMatPowerData(PFDATA *pf, char *filename, char *bus_data_str, int bus_len, char *gen_data_str, int gen_len, char *branch_data_str, int branch_len)
{
  PetscErrorCode ierr;
  VERTEX_Power   Bus;
  LOAD           Load;
  ...
  pf->nbus    = bus_len;
  pf->ngen    = gen_len;
  pf->nbranch = branch_len;

  //char *token = strtok(bus_data_str, ";");
  char *token, *strcopy, *tofree;
  tofree = strcopy = strdup(bus_data_str);
  /* Count the number of pq loads */
  while((token = strsep(&strcopy, ";"))) //while(token != NULL)
  {
    sscanf(token,"%d %d %lf %lf",&extbusnum,&bustype_i,&Pd,&Qd);
  ...
  printf("Reading data from 3 arrays.....\n");
  //printf("BUS_DATA_ARRAY: %s, \n", bus_data_str);
  //token = strtok(bus_data_str, ";");
  tofree = strcopy = strdup(bus_data_str);
  while((token = strsep(&strcopy, ";")))//while(token != NULL)
  {
      ...
      int    bus_i,ide,area;
      /* Bus data */
      sscanf(token,"%d %d %lf %lf %lf %lf %d %lf %lf %lf",&bus_i,&ide,&Pd,&Qd,&gl,&bl,&area,&vm,&va,&basekV);
      ...
      //printf("Bus[busi].bus_i: %d, busext2intmap[Bus[busi].bus_i]: %d\n", Bus[busi].bus_i, busi);
  ...
  printf("Read in the bus data.....\n");
  //token = strtok(gen_data_str, ";");
  tofree = strcopy = strdup(gen_data_str);
  while((token = strsep(&strcopy, ";")))//while(token != NULL)
  {
    //printf("Line of generator data: %s\n", token);

    double pg,qg,qt,qb,vs,mbase,pt,pb;
    int    bus_i,status;
    sscanf(token,"%d %lf %lf %lf %lf %lf %lf %d %lf %lf",&bus_i,&pg,&qg,&qt,&qb,&vs,&mbase,&status,&pt,&pb);
    ...
    //printf("Gen: Checkpoint 1: geni:%d,  Gen[geni].bus_i:%d  Gen[geni].status: %d \n", geni, Gen[geni].bus_i, Gen[geni].status);
    intbusnum = busext2intmap[Gen[geni].bus_i];
    //printf("Gen: Checkpoint 2: geni:%d, Gen[geni].internal_i:%d, intbusnum:%d \n", geni, Gen[geni].internal_i, intbusnum);
    Gen[geni].internal_i = intbusnum;
    Bus[intbusnum].gidx[Bus[intbusnum].ngen++] = geni;

    //printf("Gen: Checkpoint 3.\n");

    Bus[intbusnum].vm = Gen[geni].vs;

    //printf("In generator %d: intbusnum: %d Bus[intbusnum].ngen/NGEN_AT_BUS_MAX: %d of %d \n", geni, intbusnum, Bus[intbusnum].ngen, NGEN_AT_BUS_MAX);
  ...
  printf("Read in the generator data.....\n");
  //token = strtok(branch_data_str, ";");
  tofree = strcopy = strdup(branch_data_str);
  while((token = strsep(&strcopy, ";")))//while(token != NULL)
  {
    PetscScalar R,X,Bc,B,G,Zm,tap,shift,tap2,tapr,tapi;
    double      r,x,b,rateA,rateB,rateC,tapratio,phaseshift;
    int         fbus,tbus,status;
    sscanf(token,"%d %d %lf %lf %lf %lf %lf %lf %lf %lf %d",&fbus,&tbus,&r,&x,&b,&rateA,&rateB,&rateC,&tapratio,&phaseshift,&status);
  ...
  printf("Read in the branch data.....\n");
